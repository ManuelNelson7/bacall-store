            ...
            <div>
                <div className="max-w-2xl mx-auto py-16 px-4 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8 lg:py-32 lg:grid lg:grid-cols-2 lg:gap-x-8 xl:gap-x-24">
                    <div className="lg:col-start-2">
                        ...
                        <p className="mt-2 text-4xl font-extrabold tracking-tight text-gray-900 text-gold font-lora">Thanks for ordering</p>
                        ...
                        >
                            {ticket.items.map((product) => (
                                <li key={product.id} className="flex py-6 space-x-6">
                                    ...
                                    <p className="flex-none font-semibold text-gray-900">${product.price}</p>
                                </li>
                            ))}
                        </ul>
                        ...
                        <dl className="text-sm font-medium text-gray-500 space-y-6 border-t border-gray-200 pt-6">
                            <div className="flex justify-between">
                                <dt>Subtotal</dt>
                                <dd className="text-gray-900">${ticket.subtotal.toFixed(2)}</dd>
                            </div>

                            <div className="flex justify-between">
                                <dt>Shipping</dt>
                                <dd className="text-gray-900">{ticket.shipping === 0 ? "Free" : `$${ticket.shipping.toFixed(2)}`}</dd>
                            </div>

                            <div className="flex justify-between">
                                <dt>Taxes</dt>
                                <dd className="text-gray-900">${ticket.taxes.toFixed(2)}</dd>
                            </div>

                            <div className="flex items-center justify-between border-t border-gray-200 text-gray-900 pt-6">
                                <dt className="text-base">Total</dt>
                                <dd className="text-base font-semibold">${ticket.total.toFixed(2)}</dd>
                            </div>
                        </dl>
                            ...
                            <div>
                                ...
                                <dd className="mt-2">
                                    <address className="not-italic">
                                        <span className="block">{ticket.buyer.name}</span>
                                        <span className="block">
                                            {ticket.buyer.apartment ?
                                                `${ticket.buyer.address}, ${ticket.buyer.apartment}`
                                                :
                                                ticket.buyer.address}
                                        </span>
                                        <span className="block">{ticket.buyer.phone}</span>
                                    </address>
                                </dd>
                            </div>
                            <div>
                        ...
                        <div className="mt-16 border-t border-gray-200 py-6 text-right">
                            <Link to="/categories" className="text-md font-medium text-indigo-600 hover:text-gold transition-all duration-150">
